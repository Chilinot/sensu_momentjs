...
// ------------------------ END MOMENTJS-TIMEZONE ------------------------



// ------------------------ BEGIN CUSTOM CODE ------------------------

// Will parse the given unix epoch timestamp and convert it to the given timezone.
// Then compare the hour to see if it is withing the range defined by start and end.
// Where start <= t < end
function within_tz(tz, timestamp, start, end) {
    var m = moment.unix(timestamp).tz(tz)

    return m.hour() >= start && m.hour() < end
}
